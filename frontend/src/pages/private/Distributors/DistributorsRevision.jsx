 ...
 */
const DistributorsRevision = ({ assignmentCount }) => {
  ...
  const fetchDistributors = useCallback(async (variables) => {
    ...
    try {
      ...
      const response = await DistributorService.getMyAssignedDistributors({ 
        ...cleanedFilters,
        ...variables 
      });
      setTotalCount(response.data.data.distributorsTotalCountRevisionCorrection|| 0);
      const { edges, pageInfo: newPageInfo } = response.data.data.myAssignedDistributors;
      console.log("# distribuidores:", response.data.data);
  ...
  return (
    <Box>
      <Card borderRadius="xl" boxShadow="lg">
        <CardHeader borderBottomWidth="1px" p={4}>
          <Flex direction={{ base: 'column', md: 'row' }} justify="space-between" align={{ base: 'flex-start', md: 'center' }} gap={4}>
            <Box>
              <Heading size="lg">Asignación de Distribuidores (Revisión)</Heading>
              <Text color={textColor}>Distribuidores asignados a mi perfil.</Text>
            </Box>
     ...
     <GenericModal
      ...
      >
        <Text>
            ¿Estás seguro de que deseas eliminar el cliente "{selectedDistributor?.nombres} {setSelectedDistributor?.apellidos}"?
...
export default DistributorsRevision;