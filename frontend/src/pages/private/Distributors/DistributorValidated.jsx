 ...
 */
const DistributorValidated = ({ assignmentCount }) => {
  ...
  const fetchDistributors = useCallback(async (variables) => {
    ...
    try {
      ...
      const response = await DistributorService.getMyAssignedDistributorsValidated({ ...cleanedFilters, ...variables });
      console.log('**Distribuidores obtenidos:', response.data.data);
      
      // Verificar que la respuesta tenga la estructura esperada
      if (!response.data.data.myAssignedDistributorsValidated) {
        throw new Error('No se recibieron distribuidores asignados');
      }

      setTotalCount(response.data.data.distributorsTotalCountValidated || 0);
      const { edges, pageInfo: newPageInfo } = response.data.data.myAssignedDistributorsValidated;
      ...
      <Card borderRadius="xl" boxShadow="lg">
        <CardHeader borderBottomWidth="1px" p={4}>
          <Flex direction={{ base: 'column', md: 'row' }} justify="space-between" align={{ base: 'flex-start', md: 'center' }} gap={4}>
            <Box>
              <Heading size="lg">Asignación de Distribuidores (Validación)</Heading>
...
export default DistributorValidated;