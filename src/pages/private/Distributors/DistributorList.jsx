// frontend/src/pages/private/Distributors/DistributorList.jsx
...
/**
 * `DistributorList`
 * Componente que renderiza la lista, filtros y paginación para una vista de distribuidores.
 *
 * @param {object} props
 * @param {string} props.title - (NUEVO) El título para mostrar en el CardHeader (ej. "Pendientes").
 * @param {string} props.viewType - El identificador de la vista (ej. 'pending', 'my_revisions').
 * @param {boolean} props.isTrackingView - Flag para saber si debe renderizar la vista de Tracking.
 * @param {Function} props.onAssignmentSuccess - Callback para notificar al padre de una asignación.
 */
// REFACTOR: Aceptar 'title' como prop
const DistributorList = ({ title, viewType, isTrackingView, onAssignmentSuccess }) => {
  ...
  // REFACTOR: Esta función se elimina porque 'tabConfig' no existe aquí.
  // const getTitle = () => {
  //   const tab = tabConfig.find(t => t.viewType === viewType); // <-- ERROR OCURRÍA AQUÍ
  //   return tab ? tab.label : "Distribuidores";
  // };
  
  const getSubTitle = () => {
    // Esta función es segura porque solo depende de 'viewType' (prop)
    const messages = {
    ...
    <Box>
      <Card borderRadius="xl" boxShadow="lg">
        <CardHeader borderBottomWidth="1px" p={4}>
          ...
          >
            <Box>
              {/* REFACTOR: Usar el prop 'title' directamente */}
